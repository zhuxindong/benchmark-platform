...
def create_jwt_token(data: dict) -> str:
    ...
    to_encode.update({"exp": expire.timestamp()})  # 转换为时间戳

    # 简单的token编码（生产环境应使用真正的JWT库）
...
def verify_token(token: str) -> Optional[dict]:
    """验证令牌"""
    try:
        # print(f" 开始验证token: {token[:50]}...")

        if '.' not in token:
            print("DEBUG: Token格式错误，缺少'.'分隔符")
            return None

        payload_part, signature = token.rsplit('.', 1)
        expected_signature = hashlib.md5(SECRET_KEY.encode()).hexdigest()

        # print(f" 提取的signature: {signature}")
        # print(f" 期望的signature: {expected_signature}")

        if signature != expected_signature:
            print("DEBUG: 签名验证失败")
            return None

        # payload_part 是一个JSON字符串，直接解析
        # print(f" 解析payload: {payload_part}")
        ...
        if exp:
            ...
            # print(f" 过期时间: {exp_time}, 当前时间: {current_time}")
